-- Add RBAC role check to all RPC functions.
-- Users must have app_metadata.role = 'admin' or 'analyst' to access dashboard data.
-- Set roles via Supabase dashboard or Admin API:
--   supabase.auth.admin.updateUserById(uid, { app_metadata: { role: 'admin' } })

-- Helper function to check dashboard access
CREATE OR REPLACE FUNCTION public.nf_check_dashboard_access()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.nf_check_dashboard_access() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.nf_check_dashboard_access() TO authenticated;

-- Update all existing RPC functions to use the shared RBAC check
-- nf_stats_overview
...
AS $$
BEGIN
  PERFORM nf_check_dashboard_access();
...
-- nf_stats_trend (also includes the performance optimization: time-range filter on subqueries)
...
BEGIN
  ...
  v_start := CURRENT_DATE - v_days * INTERVAL '1 day';
  ...
  FROM generate_series(v_start::timestamp, CURRENT_DATE, INTERVAL '1 day') AS d(date)
  LEFT JOIN (
...
-- nf_stats_transfer_success_rate
...
-- New: VIP-specific success rate function
CREATE OR REPLACE FUNCTION public.nf_ai_vip_success_rate(
  p_from timestamptz DEFAULT NULL,
  p_to timestamptz DEFAULT NULL
)
RETURNS TABLE (
...
REVOKE ALL ON FUNCTION public.nf_ai_vip_success_rate(timestamptz, timestamptz) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.nf_ai_vip_success_rate(timestamptz, timestamptz) TO authenticated;
