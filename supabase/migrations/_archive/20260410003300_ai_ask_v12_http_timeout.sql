-- nf_ai_ask v12.1 — raise extensions.http timeout from default 5s to 30s.
--
-- Gemma 4 via Google AI Studio cold-starts occasionally take 6-10s, which
-- was blowing up Call 1 with "Operation timed out after 5002 milliseconds".
-- CURLOPT_TIMEOUT_MS is per-session, so we set it at the top of nf_ai_ask.
--
-- Body is otherwise identical to 20260410003200_ai_ask_v12_gemma.sql —
-- only the single PERFORM line after BEGIN is new.
...
DECLARE
  ...
  v_err_reason       text;
BEGIN
  -- v12.1: give Gemma 4 30s to respond (default is 5s, cold starts exceed it).
