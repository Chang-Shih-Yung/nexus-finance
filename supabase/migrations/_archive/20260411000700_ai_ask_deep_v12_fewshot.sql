-- nf_ai_ask_deep v1.2 — few-shot example + prefill + degraded flag.
--
-- Changes from v1.1 (zhtw_prefill):
--   * Adds a concrete few-shot example in the user turn so Gemma has a
--     pattern to mimic rather than a rules list to (mis)interpret as a
--     document. Instruction-tuned models follow "show one, copy the
--     shape" far better than "follow these 5 rules".
--   * Adds `degraded: boolean` + `degrade_reason: text` to the return
--     jsonb so the frontend can warn when parsing fell through and we
--     dumped the raw model text into `trend`.
--   * Keeps the v1.1 prefill trick (role: model turn seeded with
--     "【趨勢】\n") which was the biggest single reliability win.
--
-- Degrade reasons exposed:
--   http_failed           - extension.http raised
--   http_status_<code>    - non-200 from Google AI
--   invalid_json          - response body wasn't JSON
--   empty_response        - 200 with no text part
--   parse_failed          - model output didn't contain any 【section】
--                           labels; frontend should render the raw
--                           `trend` text as-is (content still useful,
--                           just unstructured).
...
BEGIN
  ...
  -- Strategy: data first, instructions last, a WORKED EXAMPLE in
  -- the middle so Gemma pattern-matches shape instead of parsing
  -- a rules list. The example uses a different domain (放款組合)
  -- so the model can't just copy words — it has to copy structure.
  v_prompt := format(
...
E'4. 不要前言、不要重述題目、不要複述範例。直接從「【趨勢】」開始寫。',
  ...
  -- Prefill with a model turn so Gemma resumes from "【趨勢】\n"
  -- rather than starting its own preamble.
  ...
  -- ── Fallback: parser found nothing → degraded + full raw text ──
  ...
  -- Also flag as degraded if some sections parsed but not all (≥1 missing
  -- among trend/obs/recs — risks is optional by product rule).
  IF v_degraded = false
