-- nf_ai_ask_deep v1.4 — switch narration model from gemma-4-31b-it to
-- gemma-4-26b-a4b-it (same one Call 1 uses).
--
-- Why: v1.3 was hitting `http_failed: Operation timed out after 90000
-- milliseconds with 0 bytes received` on the 31B free-tier endpoint.
-- Postgres statement_timeout was already lifted to 120s via function
-- attribute (migration 20260411001000) so that's not the bottleneck —
-- Google's 31B cold-start + inference was simply exceeding our 90s
-- curl ceiling before sending a single byte.
--
-- Going to 26B A4B solves two things at once:
--   1. 26B is faster end-to-end on the free tier, especially for
--      ≤1400-token outputs like our four-section deep analysis.
--   2. Call 1 (which already uses 26B) usually warms Google's endpoint
--      for that model in the preceding 1-2s, so by the time we fire
--      Call 2 → Deep, the model is hot and first-byte latency drops
--      dramatically.
--
-- No other changes. Prompt, prefill, few-shot, parser, degraded flag
-- logic, curl timeout (90s), and statement_timeout (120s via function
-- attribute in migration 20260411001000) are all preserved. Only the
-- v_model variable and the header version string move.
...
DECLARE
  v_api_key      text;
  v_model        text := 'gemma-4-26b-a4b-it';  -- was gemma-4-31b-it in v1.3
  v_endpoint     text;
  v_existing_sub text;
  v_masked       jsonb;
  v_row_count    int;
