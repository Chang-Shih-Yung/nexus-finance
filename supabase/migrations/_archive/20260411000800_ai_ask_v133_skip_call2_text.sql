-- nf_ai_ask v13.3 — skip Call 2 on single-row/text, add prefill+few-shot
-- to Call 2 for multi-row, bump statement_timeout to 120s.
--
-- Problems addressed:
--   1. v13.2 tripped the degraded badge on single-row KPI queries like
--      "目前放款組合與 NPL 比率" because Call 2 fired even though the
--      narration adds zero value for a 6-KPI card. False alarm.
--      → Fix: skip Call 2 entirely when render resolves to 'text' or
--              the result has ≤1 row. Use template narration WITHOUT
--              flagging degraded. Execs see a clean bubble.
--   2. Call 2 parsing still uses raw "SUMMARY:/INSIGHT:" labels with
--      no prefill or worked example, so Gemma 4 31B slips into other
--      formats and the parser misses.
--      → Fix: bring the same prefill + few-shot technique that fixed
--              nf_ai_ask_deep down to Call 2:
--                • zh-TW labels 【摘要】/【觀察】 (full-width brackets)
--                • one worked example showing the shape
--                • role: "model" prefill seeded with "【摘要】\n"
--                • tolerant parser accepting 【】 or [] and markdown bold
--   3. Deep analysis was hitting PostgREST statement_timeout (8s default
--      for authenticated role) before Gemma could respond, manifesting
--      as "canceling statement due to statement timeout" — NOT the curl
--      timeout we set via CURLOPT_TIMEOUT_MS.
--      → Fix: SET LOCAL statement_timeout = '120s' at the top of the
--              function. Only affects this call, no role/DB pollution.
...
BEGIN
  -- PostgREST/`authenticated` role defaults statement_timeout to 8s which
  -- cuts Gemma calls off mid-flight. Bump to 2 minutes for this call only.
  ...
  -- Chart was catalog-requested but could not be derived → degrade to
  -- table but DO flag it so the user knows the chart was dropped.
  ...
  -- Rationale: KPI grids are self-explanatory; running Call 2 just to
  -- generate a one-line "以下是...最新數據" intro wastes Gemma RPD and
  -- produces false degraded badges when the model goes off-format.
  -- ══════════════════════════════════════════════════════════════════
  ...
  -- ══════════════════════════════════════════════════════════════════
  -- Call 2 — narration with prefill + few-shot (multi-row only)
  -- ══════════════════════════════════════════════════════════════════
...
E'5. 直接從「【摘要】」開始寫，不要前言、不要複述範例。',
    ...
    IF v_raw_text IS NOT NULL AND v_raw_text <> '' THEN
      FOREACH v_line IN ARRAY string_to_array(v_raw_text, E'\n') LOOP
        ...
        -- Legacy fallbacks: if the model slipped and used SUMMARY/INSIGHT
        -- English labels, still accept them so we don't waste the call.
