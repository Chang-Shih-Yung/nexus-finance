-- nf_ai_ask v13.1 — fix rank-column y-axis picking.
--
-- Bug: when the RPC returns `{rank, branch, actual, target, achievement_rate}`
-- (e.g. nf_branch_ranking), the chart derivation picked v_number_fields[1]
-- which happened to be `rank`. Result: bar chart showed 新竹=1 as the
-- *shortest* bar while actually it was the top performer.
--
-- Fix: after collecting numeric fields, push rank-like columns
-- (rank, id, seq, order, index, no, num, 排名) to the BACK of
-- v_number_fields so the first pick is always a "real" metric column.
-- Fallback chain still works: if rank is the only numeric column, it
-- stays in position 1 and the chart still renders (better than nothing).
--
-- Only the field-collection block changes; everything else is a verbatim
-- CREATE OR REPLACE of the v13 function from 20260411000200.
...
BEGIN
  ...
  IF v_hint <> 'none'
    ...
    -- ── v13.1 fix: push rank-like numeric columns to the back ──
    -- Otherwise nf_branch_ranking returns {rank, branch, actual, …} and
    -- rank (1,2,3,…) gets picked as y-axis, which inverts the visual
    -- story (winner looks shortest). We rank candidates by a preference
    -- score: ordering/identifier-ish names score 1 (deprioritized), all
    -- else score 0, ties broken by original column order.
