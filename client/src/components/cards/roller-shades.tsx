    ...
    <Card>
      ...
      <CardContent className="flex flex-col gap-4">
        ...
        <div className="flex items-center gap-3">
          <span className="text-xs font-medium tracking-wider text-muted-foreground uppercase">Open</span>
          <Slider value={position} onValueChange={(value) => setPosition(Array.isArray(value) ? [...value] : [value])} max={100} className="flex-1" />
          <span className="text-xs font-medium tracking-wider text-muted-foreground uppercase">Close</span>
        </div>
      </CardContent>
      <CardFooter>
        <ToggleGroup
        ...
        >
          ...
          <ToggleGroupItem value="half" className="flex-1">Half</ToggleGroupItem>
          <ToggleGroupItem value="closed" className="flex-1">Closed</ToggleGroupItem>
        </ToggleGroup>
      </CardFooter>
    </Card>
  )
