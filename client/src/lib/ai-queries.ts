...
async function queryVipSuccessRateThisMonth(): Promise<AiQueryResponse> {
    const rows = await invokeRpc<Array<{ date: string; success_rate: number; total: number }>>('nf_ai_vip_success_rate', {
    ...
    return {
        ...
        sql: "SELECT DATE(created_at), COUNT(*) FILTER (WHERE status='success') / COUNT(*) FROM transactions JOIN users ON ... WHERE tier='vip' ...",
        chartConfig: {
            type: 'line',
            data: {
...
export async function runAiTemplateQuery(query: string): Promise<AiQueryResponse> {
